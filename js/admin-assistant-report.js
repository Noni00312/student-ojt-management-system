document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    if (userId) {
      ...
      showError("No user ID found. Please return to the assistant list.");
    }
  } catch (error) {
...
function createLoader() {
  try {
    ...
    const container =
      document.querySelector(".student-report-container") || document.body;
...
function showError(message) {
  const container = document.querySelector(".student-report-container");
  container.innerHTML = `
...
async function loadStudentData(userId) {
  ...
  try {
    ...
    if (emailTooltipMobile) {
      ...
      initTooltip(emailTooltipMobile, "top");
    }
...
async function loadAssistantReports(userId) {
  ...
  try {
    ...
    const assistants = await firebaseCRUD.queryData(
      "students",
      "userId",
      "==",
      userId
